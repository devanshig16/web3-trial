...
export default function BrowseCompanies() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      <h2 className="text-3xl font-bold mb-6 text-center text-black">Browse Companies</h2>
      {loading ? (
        <p className="text-center text-black">Loading companies...</p>
      ) : companies.length === 0 ? (
        <p className="text-center text-gray-500">No companies listed yet.</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {companies.map((company) => (
            <div key={company.id} className="p-4 border rounded-lg shadow-sm bg-white space-y-2">
